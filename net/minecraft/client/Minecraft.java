...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        Honertis.INSTANCE.update = WebUtils.update();
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle(Honertis.INSTANCE.title);
    ...
    private void setWindowIcon()
    {
        ...
        if (util$enumos != Util.EnumOS.OSX)
        {
            ...
            try
            {
                inputstream = this.mcDefaultResourcePack.getInputStreamAssets(new ResourceLocation("items/bowl.png"));
                inputstream1 = this.mcDefaultResourcePack.getInputStreamAssets(new ResourceLocation("items/bowl.png"));
    ...
    private void startTimerHackThread()
    {
        Thread thread = new Thread("Timer hack thread")
        {
            public void run()
            {
                while (Minecraft.this.isRunning())
                {
                    try
                    {
    ...
    {
        return Display.isActive() ?	this.gameSettings.limitFramerate : 30;
    }
        ...
        if (this.leftClickCounter <= 0)
        {
        	if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
            {
                ...
                if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air)
                {
                	...
                	if (Honertis.INSTANCE.getModule(BlockTap.class).isEnabled() && thePlayer.isUsingItem()) {
                		this.effectRenderer.addBlockHitEffects(blockpos, this.objectMouseOver.sideHit);
    	                this.thePlayer.swingItem();	
                	}
                }
            ...
            while (Mouse.next())
            {
                ...
                if (i1 <= 200L)
                {
                    ...
                    if (j != 0)
                    {
                        ...
                        {
                        	Zoom zoom = Honertis.INSTANCE.getModule(Zoom.class);
                        	if (!zoom.isZooming)
